        ...
        <div class="grid-booking">
          <template v-for="(el, idx) in ructen" :key="idx">
            ...
            <a-form-item class="item-max" v-if="el.type === 'iSelect'" :name="el.name" :label="el.label"
              ...
              <a-select v-if="el.name == 'detailprocedure_id'" v-model:value="formState[el.name]"
                :options="store.detailProcedures" />
...
const rucValidator = (rule, value) => {
  return new Promise((resolve, reject) => {
    if (!value || value.trim() === '') {
      resolve();
    } else if (isNaN(value) || value < 10000000000 || value > 19999999999) {
      reject(new Error('El número de RUC no es válido'));
    } else {
      resolve();
    }
  });
